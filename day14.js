...
console.log('After')
var residue_arr = []
var initial_inp = JSON.parse(JSON.stringify(fuel_inp))
function produce(multi){
	var running = true;
	fuel_inp = JSON.parse(JSON.stringify(initial_inp));
	fuel_inp["inps"].map(function(item){
    	item["num"] = item["num"]*multi;
    	return item;
	})
	residue_arr = []
	while(running){
				...
				out_iter["outs"].forEach(function(chain_outs){
					if(fuel_inp["inps"][index] && fuel_inp["inps"][index]["elem"] && chain_outs["elem"] == fuel_inp["inps"][index]["elem"]){
						...
						out_iter["inps"].forEach(function(out_iter_inps){
							...
							var accumulated = 0;
							var deduct = 0;
							var match_deduct = residue_arr.map(function(item){return Object.keys(item)[0]}).indexOf(out_iter_inps["elem"])
							if(match_deduct > -1){
								deduct = residue_arr.splice(match_deduct, 1)[0][out_iter_inps["elem"]]
							}
							if(match > -1){
	...
	return fuel_inp["inps"][0]["num"];
}
// var ore_sum = 30000
var ore_sum = 1000000000000
var fuel_count = 1;
var produced = 0;
var multiple = 10000;
console.log('part 1:', produce(1))
while(true){
	multiple = multiple / 10
	while(ore_sum > produced){
		produced = produce(fuel_count)
		// console.log(fuel_count, 'FUEL requires', produced, 'ORES')
		fuel_count += multiple
	}
	produced = 0;
	fuel_count -= multiple*2
	if(multiple == 1){
		break;
	}
}
console.log('part 2:', fuel_count)
