	...
	for(var index=0; index<length_fuel_inps; index++){
		output.forEach(function(out_iter){
			...
			out_iter["outs"].forEach(function(chain_outs){
				// console.log(fuel_inp["inps"][index] && fuel_inp["inps"][index]["elem"])
				if(fuel_inp["inps"][index] && fuel_inp["inps"][index]["elem"] && chain_outs["elem"] == fuel_inp["inps"][index]["elem"]){
					// debugger
					var replacement = fuel_inp["inps"].splice(index, 1)[0]["num"]
					
					replacement = replacement/chain_outs["num"] < 1 ? 1 : Math.ceil(replacement/chain_outs["num"])
					out_iter["inps"].forEach(function(out_iter_inps){
						var match = fuel_inp["inps"].map(function(item){return item["elem"]}).indexOf(out_iter_inps["elem"])
						var accumulated = 0;
						if(match > -1){
							accumulated = fuel_inp["inps"].splice(match, 1)[0]["num"]
						}
						fuel_inp["inps"].push({
							"num": out_iter_inps["num"]*replacement + accumulated,
...
fuel_inp["inps"].forEach(function(item){
	console.log(Math.ceil(item["num"]))
